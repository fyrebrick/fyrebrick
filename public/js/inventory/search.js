let searched = [];
let lastRemarksChange = 0;
let lastQuantityChange = 0;
let lastUsedChange = 0;
let delay = 100;
$(document).ready(function(){
    sortableTableId = "mainTable";
    addSortIcons();
    document.querySelectorAll("#mainTable th.sortable").forEach(function (item){
        item.addEventListener("click",sortTable)
    })
    $("#0").click();
    searchButtonListener();
})


function searchButtonListener(){
    document.getElementById('search').addEventListener('click', function (e) {
        document.getElementById('search').focus();
        document.getElementById('search').select();
    });
    document.addEventListener('onsubmit', function (e) {
        document.getElementById("searchButton").click();
    });
    document.getElementById('search')
        .addEventListener('keyup', function (e) {
            if (e.key === 'Enter') {
                document.getElementById("searchButton").click();
                document.getElementById('search').focus();
                document.getElementById('search').select();
            }
        });
    document.getElementById("searchButton").addEventListener('click',search);
}
function listenersWhenSearchIsComplete (){
    document.querySelectorAll("#mainTable th.sortable").forEach(function (item){
        item.addEventListener("click",sortTable)
    })
    document.querySelectorAll(".inputRemarks").forEach(function (item) {
        item.addEventListener('keyup', function (e) {
            if (e.key === 'Enter') {
                changeRemarks(e);
                document.getElementById('search').focus();
                document.getElementById('search').select();
            }
        });
        item.addEventListener('click', function (e) {
            e.target.focus();
            e.target.select();
        });
    });
    document.querySelectorAll(".inputQuantity").forEach(function (item) {
        item.addEventListener('keyup', function (e) {
            if (e.key === 'Enter') {
                changeQuantity(e);
                document.getElementById('search').focus();
                document.getElementById('search').select();
            }
        });
        item.addEventListener('click', function (e) {
            e.target.focus();
            e.target.select();
        });
    });
    document.querySelectorAll(".inputColour").forEach(function (item) {
        item.addEventListener('change', function (e) {
            changeColour(e);
            document.getElementById('search').focus();
            document.getElementById('search').select();
        });
    });
    document.querySelectorAll(".inputUsed").forEach(function (item) {
        item.addEventListener('keyup', function (e) {
            if (e.key === 'Enter') {
                changeUsed(e);
                document.getElementById('search').focus();
                document.getElementById('search').select();
            }
        });
        item.addEventListener('click', function (e) {
            e.target.focus();
            e.target.select();
        });
    });

    document.querySelectorAll("img").forEach(function (item) {
        item.addEventListener('click', imgThumbnail);
    });
    document.querySelectorAll(".qty").forEach(function (item) {
        item.addEventListener('click', qty);
    });
}
function changeUsed(e){
    e.preventDefault();
    if (lastUsedChange >= (Date.now() - delay))
    return;
    lastUsedChange = Date.now();
    let id = e.target.id.substr(1).trim();
    let thisButton = "#" + e.target.id;
    let newUsed = $(thisButton).val();
    $.ajax({
        method: "POST",
        url: '/my/inventory/update/new_or_used',
        data: {
            inventory_id: id,
            new_or_used: newUsed
        },
        beforeSend: function () {
            $(thisButton).removeClass("is-valid");
            $(thisButton).removeClass("is-invalid");
        }
    }).done(function (data) {
        try{
            data = JSON.parse(data);
        if (data.meta.code !== 200||data.data.new_or_used!==newUsed) {
            $(thisButton).addClass("is-invalid");
        } else {
            $(thisButton).addClass("is-valid");
        }}catch(err){
            $(thisButton).addClass("is-invalid");
        }
    });
}

function changeQuantity(e){
    e.preventDefault();
    if (lastQuantityChange >= (Date.now() - delay))
    return;
    lastQuantityChange = Date.now();
    let id = e.target.id.substr(1).trim();
    let thisButton = "#" + e.target.id;
    let newQuantity = $(thisButton).val();
    $.ajax({
        method: "POST",
        url: '/my/inventory/update/quantity',
        data: {
            inventory_id: id,
            quantity: newQuantity
        },
        beforeSend: function () {
            $(thisButton).removeClass("is-valid");
            $(thisButton).removeClass("is-invalid");
        }
    }).done(function (data) {
        try{
            
            data = JSON.parse(data);
        if (data.meta.code !== 200||data.data.quantity!=newQuantity
        ) {
            $(thisButton).addClass("is-invalid");
        } else {
            $(thisButton).addClass("is-valid");
        }}catch(err){
            $(thisButton).addClass("is-invalid");
        }

    });
}

function changeRemarks(e) {
    e.preventDefault();
    if (lastRemarksChange >= (Date.now() - delay))
    return;
    lastRemarksChange = Date.now();
        let id = e.target.id.substr(1).trim();
        let thisButton = "#"+e.target.id;
        let newRemarkName = $(thisButton).val();
        $.ajax({
            method: "POST",
            url: '/my/inventory/update/remarks',
            data: {
                inventory_id: id,
                remarks: newRemarkName
            },
            beforeSend: function () {
                $(thisButton).removeClass("is-valid");
                $(thisButton).removeClass("is-invalid");
            }
        }).done(function (data) {
            try{
                data = JSON.parse(data);
            if (data.meta.code !== 200||data.data.remarks!==newRemarkName) {
                $(thisButton).addClass("is-invalid");
            } else {
                $(thisButton).addClass("is-valid");
            }
            }catch(err){
                $(thisButton).addClass("is-invalid");
            }
        });
}

function imgThumbnail(e) {
    e.preventDefault();
    if ($(e.target).is('img.img-thumbnail')) {
        let id = e.target.parentNode.parentNode.id;
        let value;
        searched.forEach(function(item){
            console.log(item.inventory_id)
            if(item.inventory_id==id){
                value = item;
            }
        })
        if(!value){
            console.log('could not find the right id in items, '+id);
        }
        console.log(value);
        const link = "https://www.bricklink.com/v2/inventory_detail.page?invID="+value.inventory_id;
        const encode_str = value.item.name.replaceAll('&#40;','(').replaceAll("&#41;",")");
        $("#enlargedTitleLabel").text("Item no. "+value.item.no+" price: "+Number(value.unit_price).toFixed(2)+" EUR");
        $("#enlargedFooter").text(unescape(encode_str)); //does not want to unescape
        $("#enlargedImg").attr('src',e.target.src);
        $("#enlargedItemLink").attr('href',link)
        $('#enlarge').modal('show');
    }
}

function search(e) {
    e.preventDefault()
    searchValue = ""
    $("#search").val($("#search").val().toUpperCase());
    $("#bodyOfTable tr").remove();
    let url_ = '/my/inventory/search?s=' + $("#search").val();
    if($("#scanoption").is(':checked')){
        url_= '/my/inventory/search?s=' + $("#search").val()+"&exact=Y";
    }
    $.ajax({
        method: "POST",
        url: url_,
        beforeSend: function () {
            startLoading()
        }
    }).done(function (data) {
        if(!data || data.length===0){
            stopLoading();
        }
        searched = data;
        const count = data.length;
        const message = 
        `
        <div class="alert-message alert alert-success alert-dismissible fade show" role="alert">
            <span>Found ${count} items</span>
            <button class="close" id="message-close-button" type="button" data-dismiss="alert" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
        </div>
        `;
        $("#messages").append(message);
        setTimeout(function(){
            $('.alert-success').delay(1500).fadeOut('slow');
        },2000);
        data.forEach(function (item) {
            let tr = "<tr id=" + item.inventory_id + ">"; //start row
            tr+="<td >";
            tr+="<a href='https://www.bricklink.com/inventory_detail.asp?pg=1&invID=" + item.inventory_id + "' target='_blank' class=\"btn btn-primary btn-lg active\" role=\"button\" aria-pressed=\"true\"> " +
                "<i class=\"fas fa-external-link-alt fa-1x\"></i>" +
                "</a>"
            tr+="</td >";
            tr += "<td data-order='"+item.remarks+"' >"; // start 1 column
            if (item.remarks === undefined) {
                tr += "<a href='https://www.bricklink.com/inventory_detail.asp?pg=1&invID=" + item.inventory_id + "' target='_blank'  >geen remark</a>";
            } else {
                tr += "<input type=\"text\" class=\"form-control inputRemarks\" id=\"R" + item.inventory_id + "\" placeholder=\"" + item.remarks + "\" value=\"" + item.remarks + "\">\n";
            }
            tr += "</td><td data-order='"+item.color_name+"' >";//start 2nd column
            if (item.color_name === "(Not Applicable)") {
                tr += "<i class=\"fas fa-tint-slash\"></i>";
            } else {
                if (item.color_name === "(Not Applicable)") {
                    tr += "<i class=\"fas fa-tint-slash\"></i>";
                } else {
                    tr += `<span type="button" class="small-screens badge badge-pill" ${setColor(item.color_name).style} data-trigger="focus" data-container="body" data-toggle="popover" data-placement="top" data-content="${item.color_name}">
                        <i class="fas fa-tint"></i>
                    </span>`;
                    
                    tr += "<p class='big-screens' id='CM"+item.inventory_id+"'>"+ setColor(item.color_name).html +"</p>"
                }
            }
            tr += "</td><td data-order='"+item.quantity+"' >";//start 3nd column
            tr += "<input type=\"text\" class=\"form-control inputQuantity\" id=\"Q" + item.inventory_id + "\" placeholder=\"" + item.quantity + "\" value=\"" + item.quantity + "\">\n";
            tr += "</td><td data-order='"+item.new_or_used+"' >";//start 4nd column
            tr += "<input type=\"text\" class=\"form-control inputUsed\" id=\"U" + item.inventory_id + "\" placeholder=\"" + item.new_or_used + "\" value=\"" + item.new_or_used + "\">\n";
            tr += "</td><td>";//start 5nd column
            tr += getPic(item);
            tr += "</td></tr>";
            $('#bodyOfTable').append(tr);
            searchValue = $("#search").val();
            listenersWhenSearchIsComplete();
            stopLoading();
        });
    });

    function getPic(row) {
        //row param is the item
        let id = row.color_id + row.item.no;
        let _type = row.item.type.substr(0, 1);

        switch (row.item.type) {
            case "SET":
                return "<img id=\"img" + id + "\" class=\"img-thumbnail \" src=\"https://img.bricklink.com/S/" + row.item.no + ".jpg\"" + " alt=\"\"> ";
                break;
            case "MINIFIG":
                return "<img id=\"img" + id + "\" class=\"img-thumbnail \" src=\"https://img.bricklink.com/ItemImage/MN/" + row.color_id + "/" + row.item.no + ".png\"" + " alt=\"\"> ";
                break;
            case "PART":
                return "<img id=\"img" + id + "\" class=\"img-thumbnail \" src=\"https://img.bricklink.com/ItemImage/PN/" + row.color_id + "/" + row.item.no + ".png\"" + " alt=\"\"> ";
                break;
            case 'INSTRUCTION':
                return "<img id=\"img" + id + "\" class=\"img-thumbnail \" src=\"https://img.bricklink.com/ItemImage/IN/" + row.color_id + "/" + row.item.no + ".png\"" + " alt=\"\"> ";
                break;
            case 'BOOK':
                return "<img id=\"img" + id + "\" class=\"img-thumbnail \" src=\"https://img.bricklink.com/ItemImage/BN/" + row.color_id + "/" + row.item.no + ".png\"" + " alt=\"\"> ";
                break;
            case 'SET':
                return "<img id=\"img" + id + "\" class=\"img-thumbnail \" src=\"https://img.bricklink.com/ItemImage/SN/" + row.color_id + "/" + row.item.no + ".png\"" + " alt=\"\"> ";
                break;
            case 'GEAR':
                return "<img id=\"img" + id + "\" class=\"img-thumbnail \" src=\"https://img.bricklink.com/ItemImage/GN/" + row.color_id + "/" + row.item.no + ".png\"" + " alt=\"\"> ";
                break;
            case 'CATALOG':
                return "<img id=\"img" + id + "\" class=\"img-thumbnail \" src=\"https://img.bricklink.com/ItemImage/CN/" + row.color_id + "/" + row.item.no + ".png\"" + " alt=\"\"> ";
                break;
            default:
                return "<i class=\"fas fa-image\"></i>";
        }

    }
};

function qty(e) {
    e.preventDefault();
    if ($(e.target).is(".qty")) {
        let data = $(this).data(); //name, id, qty
        $("#qtyTitle").text("Add or remove quantity to item \"" + data.name + "\"");
        $("#qtyNumberCurrent").text(data.qty);
        $("#qtyId").text(data.id);
        $("#qtyModal").modal('show');
    }
}

function setColor(color_name) {
    let style = "";
    switch (color_name) {
        case "transparent":
            style = "style='background-color:transparent'";
            break;
        case "Black":
            style = "style='background-color:#212121;color:#FFF'";
            break;
        case "Blue":
            style = "style='background-color:#0057A6;color:#FFF'";
            break;
        case "Bright Green":
            style = "style='background-color:#10CB31;color:#000'";
            break;
        case "Bright Light Blue":
            style = "style='background-color:#9FC3E9;color:#000'";
            break;
        case "Bright Light Orange":
            style = "style='background-color:#F7BA30;color:#000'";
            break;
        case "Bright Light Yellow":
            style = "style='background-color:#F3E055;color:#000'";
            break;
        case "Bright Pink":
            style = "style='background-color:#FFBBFF;color:#000'";
            break;
        case "Brown":
            style = "style='background-color:#532115;color:#FFF'";
            break;
        case "Dark Azure":
            style = "style='background-color:#3399FF;color:#FFF'";
            break;
        case "Dark Blue":
            style = "style='background-color:#143044;color:#FFF'";
            break;
        case "Dark Bluish Gray":
            style = "style='background-color:#595D60;color:#FFF'";
            break;
        case "Dark Gray":
            style = "style='background-color:#6B5A5A;color:#FFF'";
            break;
        case "Dark Green":
            style = "style='background-color:#2E5543;color:#FFF'";
            break;
        case "Dark Orange":
            style = "style='background-color:#B35408;color:#FFF'";
            break;
        case "Dark Pink":
            style = "style='background-color:#C87080;color:#FFF'";
            break;
        case "Dark Purple":
            style = "style='background-color:#5F2683;color:#FFF'";
            break;
        case "Dark Red":
            style = "style='background-color:#6A0E15;color:#FFF'";
            break;
        case "Dark Tan":
            style = "style='background-color:#907450;color:#FFF'";
            break;
        case "Dark Turquoise":
            style = "style='background-color:#008A80;color:#FFF'";
            break;
        case "Green":
            style = "style='background-color:#00642E;color:#FFF'";
            break;
        case "Lavender":
            style = "style='background-color:#B18CBF;color:#FFF'";
            break;
        case "Light Aqua":
            style = "style='background-color:#CCFFFF;color:#000'";
            break;
        case "Light Blue":
            style = "style='background-color:#B4D2E3;color:#000'";
            break;
        case "Light Bluish Gray":
            style = "style='background-color:#AFB5C7;color:#000'";
            break;
        case "Light Gray":
            style = "style='background-color:#9C9C9C;color:#000'";
            break;
        case "Light Lime":
            style = "style='background-color:#EBEE8F;color:#000'";
            break;
        case "Light Nougat":
            style = "style='background-color:#FECCB0;color:#000'";
            break;
        case "Light Pink":
            style = "style='background-color:#FFE1FF;color:#000'";
            break;
        case "Light Purple":
            style = "style='background-color:#DA70D6;color:#000'";
            break;
        case "Light Yellow":
            style = "style='background-color:#FFE383;color:#000'";
            break;
        case "Lime":
            style = "style='background-color:#A6CA55;color:#000'";
            break;
        case "Maersk Blue":
            style = "style='background-color:#6BADD6;color:#000'";
            break;
        case "Magenta":
            style = "style='background-color:#B52952;color:#FFF'";
            break;
        case "Medium Azure":
            style = "style='background-color:#42C0FB;color:#000'";
            break;
        case "Medium Blue":
            style = "style='background-color:#61AFFF;color:#000'";
            break;
        case "Medium Lavender":
            style = "style='background-color:#885E9E;color:#FFF'";
            break;
        case "Medium Lime":
            style = "style='background-color:#BDC618;color:#000'";
            break;
        case "Medium Nougat":
            style = "style='background-color:#E3A05B;color:#000'";
            break;
        case "Medium Orange":
            style = "style='background-color:#FFA531;color:#000'";
            break;
        case "Medium Violet":
            style = "style='background-color:#9391E4;color:#000'";
            break;
        case "Nougat":
            style = "style='background-color:#FFAF7D;color:#000'";
            break;
        case "Olive Green":
            style = "style='background-color:#7C9051;color:#FFF'";
            break;
        case "Orange":
            style = "style='background-color:#FF7E14;color:#FFF'";
            break;
        case "Pink":
            style = "style='background-color:#FFC0CB;color:#000'";
            break;
        case "Purple":
            style = "style='background-color:#A5499C;color:#FFF'";
            break;
        case "Red":
            style = "style='background-color:#B30006;color:#FFF'";
            break;
        case "Reddish Brown":
            style = "style='background-color:#89351D;color:#FFF'";
            break;
        case "Sand Blue":
            style = "style='background-color:#5A7184;color:#FFF'";
            break;
        case "Sand Green":
            style = "style='background-color:#76A290;color:#FFF'";
            break;
        case "Sand Red":
            style = "style='background-color:#8C6B6B;color:#FFF'";
            break;
        case "Tan":
            style = "style='background-color:#DEC69C;color:#000'";
            break;
        case "Very Light Orange":
            style = "style='background-color:#E6C05D;color:#000'";
            break;
        case "White":
            style = "style='background-color:#FFFFFF;color:#000'";
            break;
        case "Yellow":
            style = "style='background-color:#F7D117;color:#000'";
            break;
        case "Yellowish Green":
            style = "style='background-color:#DFEEA5;color:#000'";
            break;
        case "Trans-Clear":
            style = "style='background-color:#EEEEEE;color:#000'";
            break;
        case "Trans-Dark Blue":
            style = "style='background-color:#00296B;color:#FFF'";
            break;
        case "Trans-Light Blue":
            style = "style='background-color:#68BCC5;color:#000'";
            break;
        case "Trans-Light Orange":
            style = "style='background-color:#E99A3A;color:#000'";
            break;
        case "Trans-Neon Green":
            style = "style='background-color:#C0F500;color:#000'";
            break;
        case "Trans-Neon Orange":
            style = "style='background-color:#FF4231;color:#FFF'";
            break;
        case "Trans-Orange":
            style = "style='background-color:#D04010;color:#FFF'";
            break;
        case "Trans-Yellow":
            style = "style='background-color:#EBF72D;color:#000'";
            break;
        case "Trans-Red":
            style = "style='background-color: #9C0010;color:#FFF !important'";
            break;
        case "Chrome Gold":
            style = "style='background-color:#F1F2E1;color:#000'";
            break;
        case "Chrome Silver":
            style = "style='background-color:#DCDCDC;color:#000'";
            break;
        case "Pearl Light Gray":
            style = "style='background-color:#ACB7C0;color:#000'";
            break;
        case "Metallic Gold":
            style = "style='background-color:#B8860B;color:#FFF'";
            break;
        case "Milky White":
            style = "style='background-color:#D4D3DD;color:#000'";
            break;
        case "Speckle Black-Silver":
            css = 'style="color:#FFF;background: url(&quot;data:image/png;base64,&quot;) center center repeat rgb(0, 0, 0);"'
            break;
        case "Very Light Bluish Gray":
            style = "style='background-color:#747472;color:#FFF'"
            break;
        case "Trans-Green":
            style = "style='background-color:#188d33;color:#FFF'"
            break;
        case "Metallic Silver":
            style = 'style=\'color:#000;background: rgb(60,71,73);\n' +
                'background: -moz-linear-gradient(90deg, rgba(60,71,73,1) 0%, rgba(143,148,144,1) 35%, rgba(235,251,241,1) 100%);\n' +
                'background: -webkit-linear-gradient(90deg, rgba(60,71,73,1) 0%, rgba(143,148,144,1) 35%, rgba(235,251,241,1) 100%);\n' +
                'background: linear-gradient(90deg, rgba(60,71,73,1) 0%, rgba(143,148,144,1) 35%, rgba(235,251,241,1) 100%);\n' +
                'filter: progid:DXImageTransform.Microsoft.gradient(startColorstr="#3c4749",endColorstr="#ebfbf1",GradientType=1);\'';
            break;
    }
    const html = "<span class=\"badge\" "+style+">" + color_name + "</span>";
    return {
        html:html,
        style:style
    };
}