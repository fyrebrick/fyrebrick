...
module.exports = charts = {
    ...
    europe:async(req,res,next)=>{
            ...
            topGlobal = topGlobal.slice(0,100);
            let html = "";
            topGlobal.forEach((store,index)=>{
                ...
                html += "<tr "+((store.username===req.session?.user?.userName)?"id='myStore'":"")+">";
                    try{
                        ...
                        let change = index+1-store.rank.global[req.params.type][0].rank;
                        change = -change;
                        if(update<0){
                            update = `<span>${store.rank.global[req.params.type][0].rank-change}</span><i class='fas fa-sort-down' style='color:#f71d1d;'>${change}</i>`;
                        }else if(update>0){
                            update = `<span>${store.rank.global[req.params.type][0].rank-change}</span><i class='fas fa-sort-up' style='color:#15c74c;'>${change}</i>`;
                        }else{
                            update = `<span>${store.rank.global[req.params.type][0].rank-chamge}</span><i class='fas fa-minus' style='color:#15c74c;'>${change}</i>`;
                        }
                        html += `<td>${change}</td>`;
                    }catch(e){
    ...
    national:async(req,res,next)=>{
            ...
            topNational = topNational.slice(0,100);
...
const allCountryIDs = ["AT","BY","BE","BG","HR","CZ","DK","EE","FI","FR","DE","GI","GR","HU","IS","IE","IT","LV","LT","LU","MT","MD","MC","NL","NO","PL","PT","RO","RU","SM","RS","SK","SI","ES","SE","CH","UA","UK"];
const allCountries = [{
    countryID: "AT",
    countryName: "Austria",
